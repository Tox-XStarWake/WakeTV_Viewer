export class TwitchStreamdoubleknees extends HTMLElement {
    ...
    async initializeTwitchEmbed() {
        ...
        await this.newFrame();
        let embedElem = this.shadowRoot.querySelector('#twitch-doubleknees-embed');
    ...
    _setEmbedListeners() {
        this.embed.addEventListener(Twitch.Embed.VIDEO_READY, () => {
            this.dispatchEvent(new CustomEvent('twitch-doubleknees.video.ready', { detail: { embed: this.embed } }));
        });
        this.embed.addEventListener(Twitch.Embed.VIDEO_PLAY, sessionId => {
            this.dispatchEvent(
                new CustomEvent('twitch-doubleknees.video.play', { detail: { embed: this.embed, sessionId } }),
            );
        });
        ...
        events.forEach(ev => {
            this.embed.addEventListener(Twitch.Player[ev], async () => {
                ...
                this.dispatchEvent(
                    new CustomEvent(`twitch-doubleknees.${ev.toLowerCase()}`, { detail: { embed: this.embed } }),
                );
            });
        });
    }
    ...
    importTwitch() {
        return new Promise((resolve, reject) => {
            ...
            script.src = TwitchStreamdoubleknees.TWITCH_EMBED_URL;
    ...
    render() {
        const content = TwitchStreamdoubleknees.template.content.cloneNode(true);
    ...
    static get template() {
        ...
        template.innerHTML = `<div id="twitch-doubleknees-embed"></div>`;
        return template;
    }

    setDefaults() {
        const attributes = TwitchStreamdoubleknees.attributes;
...
if (!customElements.get('twitch-doubleknees')) {
    customElements.define('twitch-doubleknees', TwitchStreamdoubleknees);
}

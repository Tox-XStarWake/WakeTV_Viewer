export class TwitchStreamchefduckie extends HTMLElement {
    ...
    async initializeTwitchEmbed() {
        ...
        await this.newFrame();
        let embedElem = this.shadowRoot.querySelector('#twitch-chefduckie-embed');
    ...
    _setEmbedListeners() {
        this.embed.addEventListener(Twitch.Embed.VIDEO_READY, () => {
            this.dispatchEvent(new CustomEvent('twitch-chefduckie.video.ready', { detail: { embed: this.embed } }));
        });
        this.embed.addEventListener(Twitch.Embed.VIDEO_PLAY, sessionId => {
            this.dispatchEvent(
                new CustomEvent('twitch-chefduckie.video.play', { detail: { embed: this.embed, sessionId } }),
            );
        });
    ...
    importTwitch() {
        return new Promise((resolve, reject) => {
            ...
            script.src = TwitchStreamchefduckie.TWITCH_EMBED_URL;
    ...
    render() {
        const content = TwitchStreamchefduckie.template.content.cloneNode(true);
    ...
    setDefaults() {
        const attributes = TwitchStreamchefduckie.attributes;

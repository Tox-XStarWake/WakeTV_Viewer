export class TwitchStreamtheelphyys extends HTMLElement {
    ...
    async initializeTwitchEmbed() {
        ...
        await this.newFrame();
        let embedElem = this.shadowRoot.querySelector('#twitch-theelphyys-embed');
    ...
    _setEmbedListeners() {
        this.embed.addEventListener(Twitch.Embed.VIDEO_READY, () => {
            this.dispatchEvent(new CustomEvent('twitch-theelphyys.video.ready', { detail: { embed: this.embed } }));
        });
        this.embed.addEventListener(Twitch.Embed.VIDEO_PLAY, sessionId => {
            this.dispatchEvent(
                new CustomEvent('twitch-theelphyys.video.play', { detail: { embed: this.embed, sessionId } }),
            );
        });
        ...
        events.forEach(ev => {
            this.embed.addEventListener(Twitch.Player[ev], async () => {
                ...
                this.dispatchEvent(
                    new CustomEvent(`twitch-theelphyys.${ev.toLowerCase()}`, { detail: { embed: this.embed } }),
                );
            });
        });
    }
    ...
    importTwitch() {
        return new Promise((resolve, reject) => {
            ...
            script.src = TwitchStreamtheelphyys.TWITCH_EMBED_URL;
    ...
    render() {
        const content = TwitchStreamtheelphyys.template.content.cloneNode(true);
    ...
    static get template() {
        ...
        template.innerHTML = `<div id="twitch-theelphyys-embed"></div>`;
        return template;
    }

    setDefaults() {
        const attributes = TwitchStreamtheelphyys.attributes;
...
if (!customElements.get('twitch-theelphyys')) {
    customElements.define('twitch-theelphyys', TwitchStreamtheelphyys);
}

export class TwitchStreamSpaceMages extends HTMLElement {
  ...
  setQuality(quality) {
    ...
    if (!qualities.includes(quality)) {
      throw Error(
        `Quality is not valid. Valid qualities are ${qualities.join(", ")}`
      );
    }
  ...
  async initializeTwitchEmbed() {
    ...
    await this.newFrame();
    let embedElem = this.shadowRoot.querySelector("#twitch-SpaceMages-embed");
  ...
  _setEmbedListeners() {
    this.embed.addEventListener(Twitch.Embed.VIDEO_READY, () => {
      this.dispatchEvent(
        new CustomEvent("twitch-spacemages.video.ready", {
    ...
    this.embed.addEventListener(Twitch.Embed.VIDEO_PLAY, (sessionId) => {
      this.dispatchEvent(
        new CustomEvent("twitch-spacemages.video.play", {
    ...
    events.forEach((ev) => {
      this.embed.addEventListener(Twitch.Player[ev], async () => {
        ...
        this.dispatchEvent(
          new CustomEvent(`twitch-spacemages.${ev.toLowerCase()}`, {
            detail: { embed: this.embed },
          })
        );
      });
  ...
  importTwitch() {
    return new Promise((resolve, reject) => {
      ...
      script.src = TwitchStreamSpaceMages.TWITCH_EMBED_URL;
  ...
  render() {
    const content = TwitchStreamSpaceMages.template.content.cloneNode(true);
  ...
  static get template() {
    ...
    template.innerHTML = `<div id="twitch-SpaceMages-embed"></div>`;
    return template;
  }

  setDefaults() {
    const attributes = TwitchStreamSpaceMages.attributes;
  ...
  static get observedAttributes() {
    ...
    return Object.keys(attributes).filter((attr) => {
      return (
        typeof attributes[attr].watch === "undefined" || attributes[attr].watch
      );
    });
  }
}

if (!customElements.get("twitch-spacemages")) {
  customElements.define("twitch-spacemages", TwitchStreamSpaceMages);
}
